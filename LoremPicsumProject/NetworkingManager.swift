...
final class NetworkingManager {
    ...
    func  getDecodableObject<T: Decodable>(from urlString: String, completion: @escaping (T?, Error?) -> Void) {
        ...
        URLSession.shared.dataTask(with: url) { (data, response, error) in
          ...
          do {
                let decodedObject = try JSONDecoder().decode(T.self, from: data); completion(decodedObject, nil)
            } catch {
                completion(nil, error)
            }
        }.resume()
